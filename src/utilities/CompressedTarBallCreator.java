...
public class CompressedTarBallCreator {
	/**
	...
	public static void compressFiles(Collection<File> files, File output)
		throws IOException
	{
		Logger.print(LEVELS.DEBUG, "Compressing "+files.size() + " to "+output.getAbsoluteFile());
		...
		throws IOException
	{
		...
		if (file.isFile()) {
			...
			for (File childFile : file.listFiles()) {
				addFilesToCompression(taos, childFile, dir + "/" + file.getName());
			}
		}
	}
}
